...
| **GAME_SNACK_SIGNUP_URL** | `src/lib/app-config.ts` | Notion deep link for game snack signup. Optional: an empty string hides the snack line on the portal Home tab entirely. When set, it **must be updated every season** — same failure mode as WhatsApp Learn more. |
