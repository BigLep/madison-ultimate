...
**Signup Outreach (ADR 0007).** `GET /api/admin/outreach` returns every signup row with the same six checklist rows the player page shows (Final Forms from the live export via the pure match function, never the family route with its first-join side effects), a Checklist Complete flag, and caretaker and student emails. the route also selects the audience (every row not Checklist Complete, or the PlayerIDs and Full Names passed as repeated `players` query values), so `scripts/outreach-drafts.mjs` only renders the coach's plain-text template, and creates one Gmail draft per player with `gog`; `scripts/send-outreach-drafts.mjs` sends them one every five seconds and only under an explicit `GOG_GMAIL_NO_SEND=` prefix. The checklist is computed in one place on purpose; no sheet column stores it. See [docs/adr/0007-signup-outreach-per-player-drafts.md](docs/adr/0007-signup-outreach-per-player-drafts.md) and [docs/fall-2026/signup-outreach-plan.md](docs/fall-2026/signup-outreach-plan.md).
