...
Step 0, identity: family enters preferred first name, last name, birthdate (and legal first name only if different). Player Lookup finds an existing row, or (after the content-free near-match check) creates the row immediately and routes to **`/player/$playerId`**, the one canonical player page for the whole season: bookmarkable, history-friendly, and the page that grows availability and Player Info sections in Phase D. localStorage keeps a per-device, unbounded, MRU-ordered list of accessed players (PlayerID + display name only), with per-player remove (device-local only, never touches the backend row). Placement (settled 2026-08-29, see `docs/fall-2026/player-switcher-grill.md`): on `/player/$playerId` the switcher lives in a persistent header at the top of the page, replacing the old bottom-of-form "not this player?" link (which also fixed a real bug where a taller sticky Save-bar error state could hide that link entirely). On `/signup`, when the device has remembered players, the page presents an explicit two-path chooser under the intro heading: a "Your players" list of remembered players (tap to jump straight to their `/player/$playerId`, no re-entering identity) alongside the existing lookup/signup form; picking one path visually de-emphasizes the other, reversibly, until the family actually navigates or submits.
...
- Final Forms: found/not found; if found, parent signed / student signed / physical cleared shown distinctly plus physical clearance/expiration, with a last-synced timestamp and inline refresh ("Data last synchronized with Final Forms on [time]. If you have updated Final Forms since then, click here and we'll try again"), fire-and-forget with a single-flight guard (always triggers unless a run is already in progress). If not found: three honest branches — "you haven't registered in Final Forms yet" (with a register-now prompt), "our data may be stale" with last-refreshed + refresh, then "our records may not match; enter your last name and legal first name exactly as they appear in Final Forms" — plus contact fallback
...
**Deadline and lifecycle.** Published deadline: complete signup and Final Forms by end of day Tuesday, September 8, with recommend-finishing-earlier messaging since the school needs processing time. The consequence is stated plainly and is SPS rule: Final Forms not fully complete means the player cannot set foot on the field at tryouts (Sept 9 and 11). After Sept 8 the form stays open with a banner: "Tryout registration has closed. Late signups are not guaranteed. Go ahead and submit and contact the coaches at madisonultimate@gmail.com." About a week after tryouts (~Sept 18) new-player creation closes fully; the lookup keeps working all season so existing families on a new device can still reach their player page.
...
**Roster population (no manual PlayerID entry).** Superseded on 2026-09-07 by the coach sheet's [ADR 0001](../../../madison-ultimate-admin/coach-sheet-apps-script/docs/adr/0001-roster-keyed-by-signups-playerid.md): the coach sheet's Roster is now a formula-only view keyed by Signups PlayerID (one array formula lists every PlayerID, every other column joins by PlayerID, SPS Student ID, or email), so nothing is appended or written into it and there is no "Sync players from Signups" script. The two coach-side reports promised in the grill (Q1, Q5), duplicates-suspected signups and unmatched signups-vs-Final-Forms, are carried by the coach sheet's Analyze Signups menu item. The original text, kept for history:
...
Revised 2026-08-26 after the design grill; dates corrected 2026-09-01 (tryouts moved to Wed/Fri). Key dates: tryouts Wednesday Sept 9 and Friday Sept 11 (signup and Final Forms must be fully complete before tryouts; published deadline EOD Tuesday Sept 8, recommend-earlier messaging); regular practices start the week of Sept 14; SPS/DiscNW team registration closes Sept 18; first game Sept 26. Governing principle: one-pass signup; a family works through /signup exactly once and it captures everything up front.
...
- **Milestone B (target ~Sept 2): status dashboard.** Final Forms join + three signature/clearance booleans + physical clearance/expiration + seeded contact display + on-demand refresh button (single-flight workflow_dispatch); not-found guidance messaging; "data as of" timestamp. Must be live for the Sept 2-8 chase window.
- **Milestone C: retired.** Photo upload moved into A (coaches need ID photos at tryouts Sept 9 and 11).
...
Sequencing rationale: A is the whole family-facing ask in one pass before school starts; B lands at the start of the Sept 2-8 signature-chase window, which is when nagging matters most now that tryouts (not the Sept 18 registration close) are the binding deadline. D is gated on tryouts anyway.
