...
**Seeding from Final Forms.** Two field classes with opposite sync rules. *Final Forms Status* (parent signed, student signed, cleared, physical clearance/expiration) is always read live from the newest export and never copied; Final Forms is the only place a family can change it. *Seeded fields* (grade, student email/phone, parent names/emails/phones) are shown from Final Forms as "this is what we have; use it or enter something different", copied into the signup row once when the family accepts, and ours forever after, never re-synced. Everything displays unmasked so a parent can spot wrong Final Forms data (decided 2026-08-26; the name+birthdate gate is the accepted protection). Never imported or shown: home address, race, ImPACT, payment fields.
...
- Final Forms: found/not found; if found, parent signed / student signed / physical cleared shown distinctly plus physical clearance/expiration, with a "data as of [time]" stamp and an on-demand refresh button ("I believe I've completed Final Forms; check again"), fire-and-forget with a single-flight guard (always triggers unless a run is already in progress). If not found: both honest branches, "you may not have started Final Forms" (with links) and "our records may not match; enter your last name and legal first name exactly as they appear in Final Forms", plus contact fallback
...
Settled 2026-08-26. At profile save, subscribe Parent 1/2 emails via the Buttondown API with `type: "regular"`, which skips double opt-in and the confirmation email (verified against Buttondown's docs 2026-08-26; sanctioned for own-signup-flow consent). Send the `X-Buttondown-Collision-Behavior` header so profile re-saves never error on already-subscribed emails; test once before launch whether the welcome email fires for `type: regular` creates. No tags: the tagging add-on ($9/month) is skipped, and the signups sheet's metadata can recreate any segmentation later. Students are never auto-subscribed: the personal student email gets the same one-click opt-in button, and SPS addresses are never offered subscription. The dashboard shows live per-email subscription state (existing subscriber-lookup code) with one-click join/opt-out actions. Buttondown is the single mailing-list system of record (Google Groups retired in spring 2026); pay for whatever tier covers the subscriber count.
...
- `madisonultimate.org/` directory: domain plan. Apex domain on Vercel pointing at this repo; `www` 308-redirects to apex; `next.config.js` provides on-domain redirects `/info` (Fall 2026 Notion site), `/subscribe` (Buttondown), `/news` (Buttondown archive). `/privacy` is a real page in this repo.
